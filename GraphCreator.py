...
class GraphEditor:
	...
	def create_inspector(self):
		...
		self.data_label = tk.Label(self.inspector_frame, text="Данные вершины:")
		self.data_label.pack()

		self.data_entry_var = tk.StringVar()
		self.data_entry = tk.Entry(self.inspector_frame, textvariable=self.data_entry_var)
	...
	def on_change_in_inspector(self, event):
		if self.data_same_with_name:
			...
			self.data_entry_var.set(value)  # Устанавливаем значение во втором поле
	...
	def update_inspector(self, node_id=None):
		...
		if node_id:
			...
			self.data_entry.insert(0, self.graph.nodes[node_id]["data"])
			self.id_field.config(state="normal")
			self.id_field.delete(0, tk.END)
			self.id_field.insert(0, str(node_id))
			self.id_field.config(state="readonly")
